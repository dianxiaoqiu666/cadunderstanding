...
from services.planning.baselines.m10_layouts import (adapt_m09, checked_input,
    generate_candidate, validate_operations_geometry)
from services.planning.baselines.m10_comparison import corridor_plans
from services.planning.operations_routes import evaluate_routes, validate_route_evidence

EPS=1e-6
POLICY='M10-FIXED-DEMAND-PARETO-MINIMAX-REGRET-V1'
...
def _seal(value,layout,evaluation):
    ...
    if not layout.planned_corridors:
        ...
        plans=corridor_plans(value,layout.runs[0].direction_deg,layout.runs[0].depth_mm)
...
def compare_operations(value: OperationsInput, *, include_stage_layouts=False):
    ...
    def examine(identifier,stage,layout):
        ...
        evaluation=before_eval if identifier=='M09' else evaluate_routes(value,layout)
        ...
        if not any(a.kind=='BACK_TO_BACK' for a in layout.assemblies):
            record['structure_note']='本候选保留单面排：单面比较分支，或当前横向净宽不足以放置双面组合；不能称为双面方案。'
    ...
    for direction in sorted(set(req.rules.orientation_candidates)):
        for depth in sorted({t.depth_mm for t in templates}):
            ...
            variants=[('STRUCTURE_ONLY','STRUCTURE_ONLY',[], 'BACK_TO_BACK')]
            ...
            for plan in corridor_plans(value,direction,depth):
                for pattern in ('BACK_TO_BACK','CENTRAL_SINGLE'):
                    variants.append((plan['id']+'-'+pattern,'CORRIDOR_FIRST',plan['corridors'],pattern))
            for suffix,stage,corridors,pattern in variants:
    ...
    if not frontier:
        raise PlanningError('NO_OPERATIONAL_CANDIDATE','有限候选均未满足逐面可达及冻结需求下限；未降低通道或需求，不代表全局无解。')
